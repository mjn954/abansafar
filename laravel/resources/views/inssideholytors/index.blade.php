...
@section('title', 'تورهای زیارتی داخلی')

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-purple-700">🕌 تورهای زیارتی داخلی</h1>
        <a href="{{ route('inssideholytors.create') }}"
                ...
                @forelse($holytors as $index => $tor)
                    <tr class="bg-gray-50 hover:bg-purple-50 rounded-lg transition-shadow shadow-sm">
                        ...
                        <td class="px-3 py-2 flex gap-2">
                            <a href="{{ route('inssideholytors.edit', $tor->id) }}"
                               class="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded shadow">
                                ویرایش
                            </a>

                            <form action="{{ route('inssideholytors.destroy', $tor->id) }}" method="POST"
